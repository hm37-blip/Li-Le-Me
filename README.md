...
**异常重试机制**：单用户抓取使用 Spring `@Retryable` 注解，最多重试 3 次（含首次）。3 次均失败后进入 `@Recover` 方法：记录错误日志（用户 `openid` + 失败原因 + 时间戳），跳过该用户继续结算其余成员。整批结算完成后，接口返回一个 `failed_users` 列表，包含所有跳过用户的 `openid` 和对应的异常信息，便于后续排查维护。
